...
// Painel do profissional (por slug enquanto não há Supabase Auth).
// Com auth, isto vira /painel resolvendo o profissional pela sessão.
export default async function PainelPage({ params }: { params: { slug: string } }) {
    ...
    <main style={{ maxWidth: 680, margin: '0 auto', padding: '1.5rem 1.25rem 3rem' }}>
      ...
      <div style={{ ...card, marginTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        {googleConnected ? (
          ...
          <a href={`/api/google/connect?professionalId=${professional.id}`} style={btnDark}>
            Conectar
          </a>
        ) : (
...
const card: React.CSSProperties = {
  background: '#fff', border: '1px solid #e7ece9', borderRadius: 16, padding: '0.9rem 1rem',
};
...
const btnDark: React.CSSProperties = {
  background: '#111', color: '#fff', borderRadius: 10, padding: '0.45rem 0.85rem',
